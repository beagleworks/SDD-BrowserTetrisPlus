...
function getPreviewCanvas(): HTMLCanvasElement | null {
  return document.getElementById('preview-canvas') as HTMLCanvasElement | null;
}
...
function render(state: ReturnType<typeof createInitialGameState>) {
  ...
  if (preview && state.nextBlock) {
    ...
    if (pctx) drawNextBlock(pctx, state.nextBlock);
  }
...
function setupInput(getState: () => any, setState: (s: any) => void) {
...
function ensureSpawn(state: ReturnType<typeof createInitialGameState>) {
